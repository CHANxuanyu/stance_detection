...
RT_PREFIX_RE = re.compile(r"^\s*RT\s+@\w+\s*:\s*", flags=re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-denoise",
        action="store_true",
        help="Apply light text denoising (URL/@mention/RT-prefix/quote-line cleanup)",
    )
...
def denoise_text(text: str) -> str:
    text = str(text)
    # Drop quoted lines (common in reply threads) before tokenization.
...
def denoise_texts(texts: List[str], field_name: str) -> List[str]:
    cleaned: List[str] = []
    changed = 0
    for text in texts:
        new_text = denoise_text(text)
        if new_text != text:
            changed += 1
        cleaned.append(new_text)
    print(f"[INFO] denoise {field_name}: changed={changed}/{len(texts)}")
    return cleaned
...
def load_split(
    ...
    if use_denoise:
        sources = denoise_texts(sources, "source")
        replies = denoise_texts(replies, "reply")
...
def main():
    ...
    tr_sources, tr_replies, tr_labels = load_split(
        args.train_csv, args.source_col, args.reply_col, args.label_col, label2id, args.use_denoise
    )
    ...
    if args.val_csv:
        va_sources, va_replies, va_labels = load_split(
            args.val_csv, args.source_col, args.reply_col, args.label_col, label2id, args.use_denoise
        )
    ...
    if args.test_csv:
        te_sources, te_replies, te_labels = load_split(
            args.test_csv, args.source_col, args.reply_col, args.label_col, label2id, args.use_denoise
        )
    ...
    print(f"Denoise      : {args.use_denoise}")
