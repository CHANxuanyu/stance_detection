#!/usr/bin/env python3
"""
Sweep focal gamma for train_local_csv.py and summarize macro-F1.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run multiple trainings with different --focal-gamma values"
    )
    ...
    parser.add_argument("--gammas", default="1.0,1.5,2.0,2.5", help="Comma-separated focal gamma values")
    parser.add_argument(
        "--output-prefix",
        default="./checkpoints/local-aware-budget32-92-focal",
        help="Per-run output dir prefix",
    )
    parser.add_argument(
        "--summary-csv",
        default="./checkpoints/focal_gamma_sweep_summary.csv",
...
def parse_gammas(raw: str) -> List[float]:
    ...
    if not vals:
        raise ValueError("No valid gammas parsed from --gammas")
    return vals
...
def run_one(args: argparse.Namespace, gamma: float) -> Dict[str, object]:
    tag = gamma_tag(gamma)
    out_dir = f"{args.output_prefix}-g{tag}"
...
def print_leaderboard(rows: List[Dict[str, object]]) -> None:
    ...
    for row in ok_rows:
        ...
        print(
            f"gamma={g:<5} val_macro_f1={vm} test_macro_f1={tm} "
    ...
    for row in bad_rows:
        print(
            f"gamma={row.get('gamma')} failed(return_code={row.get('return_code')}) "
...
def main() -> None:
    ...
    rows = []
    for gamma in gammas:
        rows.append(run_one(args, gamma))
