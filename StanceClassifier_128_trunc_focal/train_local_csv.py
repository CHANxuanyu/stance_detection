...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="focal",
        help="Loss type: CE, class-weighted CE, or focal loss",
    )
    parser.add_argument(
        "--class-weight-power",
        type=float,
...
class ConfigurableLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        # Compute loss ourselves to support weighted CE / focal variants.
...
def main():
    ...
    if args.loss_type == "weighted_ce":
        ...
        print(f"[INFO] loss_type=focal, focal_gamma={args.focal_gamma} (no class weights)")
    else:
