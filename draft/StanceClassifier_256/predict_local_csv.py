...
def get_model_sequence_limit(model):
...
def predict(model, tokenizer, mode, sources, replies, batch_size, max_length, truncation_strategy, device):
    all_preds = []
    all_probs = []
...
def main():
    ...
    model_limit = get_model_sequence_limit(model)
    ...
    if effective_max_length != args.max_length:
        print(
            ...
            f"tokenizer_limit={tokenizer_limit}, model_limit={model_limit}, using={effective_max_length}"
        )
