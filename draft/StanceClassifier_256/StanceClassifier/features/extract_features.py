...
class Features:
    ...
    def __init__(
        ...
        loaded_tokenizer = AutoTokenizer.from_pretrained(tokenizer_PATH, **tokenizer_kwargs)
        if max_length is None:
            tok_limit = getattr(loaded_tokenizer, "model_max_length", None)
            if tok_limit is None or tok_limit > 100000:
                tok_limit = 128
            max_length = int(tok_limit)
    ...
    def extract_bert_input(self, tweet_dict, text_pair=None):
        ...
        truncation = self.truncation_strategy if text_pair is not None else True
