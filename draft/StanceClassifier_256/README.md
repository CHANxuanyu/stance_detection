...
### 1b) Extend current model to 256-length context (without changing backbone)
```bash
python train_local_csv.py \
  --train-csv ./data/rumoureval_2019/rumoureval2019_train.csv \
  --val-csv ./data/rumoureval_2019/rumoureval2019_val.csv \
  ...
  --mode aware \
  --max-length 256 \
  --extend-pos-embeddings 256 \
  --pos-init copy \
  --truncation-strategy only_first \
  --output-dir ./checkpoints/local-aware-256
```
